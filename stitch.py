...
colors = ((123, 234, 12), (23, 44, 240), (224, 120, 34), (21, 234, 190), (80, 160, 200), (243, 12, 100), (25, 90, 12),
          (123, 10, 140))
...
class Matcher():
    def __init__(self, image1: np.ndarray, image2: np.ndarray, method: Enum = Method.SIFT, threshold=2) -> None:
        """输入两幅图像，计算其特征值
    ...
    def compute_keypoint(self) -> None:
        ...
        Args:
            image (np.ndarray): 图像
        """
        feature = self.method.value(400)
        self._keypoints1, self._descriptors1 = feature.detectAndCompute(self.image1, None)
        self._keypoints2, self._descriptors2 = feature.detectAndCompute(self.image2, None)

    def match(self, max_match_lenth=20, threshold=2, show_match=False):
        ...
        self.match_points = sorted(self.matcher.match(self._descriptors1, self._descriptors2), key=lambda x: x.distance)
        ...
        max_distance = max(threshold * self.match_points[0].distance, 20)
        ...
        if show_match:
            img3 = cv2.drawMatches(
                self.image1, self._keypoints1, self.image2, self._keypoints2, self.match_points, None, flags=0)
            show_image(img3)
...
class Stitcher:
    def __init__(self,
        ...
        self.matcher = Matcher(image1, image2, method=method, threshold=match_threshold)
        self.M = np.eye(3)
    ...
    def stich(self,
        ...
        """
        self.matcher.match(max_match_lenth=max_match_lenth, show_match=show_match_point)

        if self.use_kmeans:
            self.image_points1, self.image_points2 = k_means.get_group_center(self.matcher.image_points1,
                                                                              self.matcher.image_points2)
        else:
            self.image_points1, self.image_points2 = (self.matcher.image_points1, self.matcher.image_points2)

        if use_new_match_method:
            # self.M = ransac.GeneticTransform(self.image_points1, self.image_points2).run()
            raise NotImplementedError("No new method in this file")
        ...
        # print("Good points and average distance: ",
        #       ransac.GeneticTransform.get_value(self.image_points1, self.image_points2, self.M))
        ...
        transformed_1 = cv2.warpPerspective(self.image1, self.M, (width, height))
        transformed_2 = cv2.warpPerspective(self.image2, self.adjustM, (width, height))
    ...
    def partial_transform(self):
        ...
        ne, se, sw, nw = [], [], [], []
        transform_acer = [[center, [up, 0], [up, right]], [center, [down, 0], [0, right]],
                          [center, [down, left], [0, left]], [[up, 0], [up, left], [up, left]]]
        transform_acer = [[center, [0, up], [right, up]], [center, [0, down], [right, 0]],
                          [center, [left, down], [left, 0]], [[0, up], [left, up], [left, up]]]
        ...
        # 求点最少处位置，排除零
        minmum = np.argmin(list(map(lambda x: len(x) if len(x) > 0 else 65536, [ne, se, sw, nw])))
        # 当足够少时
        min_part = (ne, se, sw, nw)[minmum]

        # debug:
        print("minum part: ", minmum, "point len: ", len(min_part), "|", list(map(len, (ne, se, sw, nw))))
        for index in min_part:
            point = self.image_points1[index]
            cv2.circle(self.image1, tuple(map(int, point)), 20, (0, 255, 255), 5)
        ...
        if len(min_part) < len(self.image_points1) / 8:
            for index in min_part:
                ...
                if distance(self.get_transformed_position(tuple(point)), self.image_points2[index]) > 10:

                    def relevtive_point(p):
                        return (p[0] - y_mid if p[0] > y_mid else p[0], p[1] - x_mid if p[1] > x_mid else p[1])

                    cv2.circle(self.image1, tuple(map(int, point)), 40, (255, 0, 0), 10)
                    ...
                    partial_M = cv2.getPerspectiveTransform(a(src_point), a(dest_point))
                    ...
                    print("Changed:", "\nM: ", partial_M, "\npart: ", minmum, "\ndistance: ",
                          distance(self.get_transformed_position(tuple(point)), self.image_points2[index]))
                    part = self.image1[boder_0:boder_1, boder_2:boder_3]

                    print(boder_0, boder_1, boder_2, boder_3)
                    for point in transform_acer[minmum]:
                        print(point)
                        cv2.circle(part, tuple(map(int, point)), 40, (220, 200, 200), 10)
                    for point in src_point:
                        print(point)
                        cv2.circle(part, tuple(map(int, point)), 22, (226, 43, 138), 8)

                    part = cv2.warpPerspective(part, partial_M, (part.shape[1], part.shape[0]))
                    cv2.circle(part, tuple(map(int, relevtive_point(other_point))), 40, (20, 97, 199), 6)
                    # show_image(part)
    ...
    def get_transformed_size(self) -> Tuple[int, int, int, int]:
        ...
        bottom = max(map(lambda x: self.get_transformed_position(x)[1], points))
    ...
    def get_transformed_position(self, x: Union[float, Tuple[float, float]], y: float = None,
